    ...
    items: [
      {
        href: "/chapters",
        label: "Chapters",
        desc: "13 chapters, basics se deep tak",
        icon: BookOpen,
      },
      {
        href: "/videos",
        label: "Video lessons",
        desc: "Khan Academy, topic-wise",
        icon: PlayCircle,
      },
    ],
  },
    ...
    items: [
      {
        href: "/papers",
        label: "Board papers",
        desc: "Asli CBSE papers, 2022-2026",
        icon: FileText,
      },
      {
        href: "/formulas",
        label: "Formula sheet",
        desc: "94 formulas, searchable",
        icon: Sparkles,
      },
    ],
  },
...
const SOLO: Item[] = [
  {
    href: "/notes",
    label: "Notes",
    desc: "Poora chapter, ek sheet par",
    icon: NotebookPen,
  },
  {
    href: "/doubt",
  ...
  return (
    <header className="no-print sticky top-0 z-50 w-full border-b border-white/5 bg-[#141318] shadow-[0_2px_20px_rgba(16,15,24,0.22)]">
      <div ref={navRef} className="mx-auto w-full max-w-6xl px-3 sm:px-5">
        {/* two clusters: everything navigational on the left, account on the
            right. Nothing floats in the leftover space in between. */}
        <div className="flex h-[54px] items-center justify-between gap-4 lg:h-[58px]">
          <div className="flex min-w-0 items-center gap-5 lg:gap-7">
            ...
            <Link href="/" className="flex shrink-0 items-center gap-2.5">
              <span
                aria-hidden
                className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-white transition hover:scale-[1.04] lg:h-9 lg:w-9"
              >
                <BookOpen size={15} className="text-[#141318] lg:hidden" />
                <BookOpen
                  size={16}
                  className="hidden text-[#141318] lg:block"
                />
              </span>
              <span className="font-display text-[15px] font-extrabold tracking-[-0.01em] text-white lg:text-[16px]">
                Himmat Rakh
              </span>
            </Link>
            ...
            <nav className="hidden items-center gap-0.5 md:flex lg:gap-1">
                ...
                return (
                  <div key={g.label} className="relative">
                    <button
                      ...
                      className={`flex items-center gap-1 rounded-full px-3.5 py-1.5 text-[13.5px] font-medium transition lg:px-4 lg:text-[14px] ${
                        active || isOpen
              ...
              {SOLO.map((s) => (
                <Link
                  key={s.href}
                  href={s.href}
                  className={`rounded-full px-3.5 py-1.5 text-[13.5px] font-medium transition lg:px-4 lg:text-[14px] ${
                    isActive(s.href)
                      ? "bg-white/12 text-white"
                      : "text-white/70 hover:bg-white/8 hover:text-white"
                  }`}
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <button
              onClick={logout}
              title="Logout"
              className="group hidden h-8 items-center gap-2 rounded-full bg-white pl-3.5 pr-3 text-[12.5px] font-semibold text-[#141318] transition hover:bg-white/90 md:flex lg:h-9 lg:pl-4 lg:pr-3.5 lg:text-[13px]"
            >
              arnav
              <LogOut
                size={13}
                className="text-[#141318]/45 transition group-hover:text-[#141318]"
              />
            </button>
            ...
            <button
              ...
              aria-label="Menu"
              className="grid h-8 w-8 place-items-center rounded-full bg-white text-[#141318] md:hidden"
            >
