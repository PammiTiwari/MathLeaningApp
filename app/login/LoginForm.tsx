...
/**
 * `quickFill` is only passed when the server is still running the default
 * credentials. With a real password configured there is nothing safe to put
 * on the button, so it simply is not rendered - the page can never advertise
 * credentials that do not work.
 */
export default function LoginForm({
  quickFill,
}: {
  quickFill: { username: string; password: string } | null;
}) {
        ...
        >
          ...
          <label className="block">
            ...
            <input
              ...
              autoFocus
              required
              placeholder={quickFill?.username ?? "username"}
          ...
          <div className="mt-5 border-t border-line pt-4">
            ...
            <button
              type="button"
              disabled={busy}
              onClick={() => { setUsername(quickFill.username); setPassword(quickFill.password); signIn(quickFill.username, quickFill.password); }}
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className="block text-[13.5px] font-bold leading-tight text-head">
                  {quickFill.username} · {quickFill.password}
                </span>
